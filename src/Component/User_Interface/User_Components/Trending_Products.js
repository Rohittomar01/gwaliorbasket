...
export default function Trending_Products() {
    ...
    const fetchAllDealsProducts = async () => {
        var result = await getData('product/fetch_all_TrendingProducts')
    ...
    const Cart = () => {
        return Product.map((item) => {
            return (
                <Paper style={{ margin: '0.7%', display: 'flex', flexDirection: 'column', background: 'linear-gradient(0deg, rgba(7,159,25,1) 13%, rgba(2,50,17,1) 75%)', borderRadius: 15, width: '11vw', height: '35vh', padding: '0.7%' }}>
                    <div style={{ height: '15%', justifyContent: 'center', alignItems: 'center', display: 'flex' }}><p style={{ marginTop: '15%', fontFamily: 'poppins', fontWeight: "bold", padding: '0.8', width: '60%', background: 'white', textAlign: 'center', color: '#35155D', borderRadius: 5 }}>Trending</p></div>
        ...
        <div>
            <div style={{ marginLeft: '2%', width: '94.5%', display: 'flex', flexDirection: 'row', justifyContent: 'space-between' }}>
                <div style={{ marginTop: '0%', }}>
                    <h3 style={{ fontFamily: 'poppins', fontWeight: 700 }}>Trending Products</h3>
                </div>
